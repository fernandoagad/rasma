...
import {
  deleteStaffDocument,
  renameStaffDocument,
  updateStaffDocumentLabel,
  updateStaffDocumentCategory,
  bulkUpdateStaffDocumentLabel,
  bulkUpdateStaffDocumentCategory,
} from "@/actions/staff";
...
// ---------------------------------------------------------------------------
// Helper functions (same as patient docs)
// ---------------------------------------------------------------------------
...
export function StaffDocumentsTab({
  ...
  // Preview state
  const [previewFile, setPreviewFile] = useState<StaffDocument | null>(null);
  ...
  // Rename dialog state
  const [renameTarget, setRenameTarget] = useState<StaffDocument | null>(null);
  const [newFileName, setNewFileName] = useState("");
  const [renaming, setRenaming] = useState(false);

  // Label dialog state
  const [labelTarget, setLabelTarget] = useState<StaffDocument | null>(null);
  const [newLabel, setNewLabel] = useState("");
  const [updatingLabel, setUpdatingLabel] = useState(false);

  // Category dialog state
  const [categoryTarget, setCategoryTarget] = useState<StaffDocument | null>(
    null
  );
  ...
  const categoryEntries = Object.entries(UI.rrhh.staffDocCategories).filter(
    ([key]) => key !== "all"
  );
  ...
  const handleUpload = useCallback(
    async (file: File) => {
      ...
      try {
        const res = await fetch(`/api/staff/${userId}/files`, {
          method: "POST",
          body: formData,
        });
  ...
  const handleDelete = async () => {
    ...
    setDeleting(true);
    const result = await deleteStaffDocument(deleteTarget.id, userId);
    if ("error" in result) {
  ...
  const handleRename = async () => {
    ...
    setRenaming(true);
    const result = await renameStaffDocument(
      renameTarget.id,
      userId,
      newFileName
    );
  ...
  const handleLabelUpdate = async () => {
    ...
    const result = await updateStaffDocumentLabel(
      labelTarget.id,
      userId,
      newLabel || null
    );
  ...
  const handleCategoryUpdate = async () => {
    ...
    const result = await updateStaffDocumentCategory(
      categoryTarget.id,
      userId,
      newCategory
    );
  ...
  const handleBulkLabel = async (label: string | null) => {
    ...
    const result = await bulkUpdateStaffDocumentLabel(ids, userId, label);
    if ("error" in result) {
      toast.error(result.error);
    } else {
  ...
  const handleBulkCategory = async (category: string) => {
    ...
    if (ids.length === 0) return;
    setBulkUpdating(true);
    const result = await bulkUpdateStaffDocumentCategory(
      ids,
      userId,
      category
    );
    ...
    <div className="space-y-4">
      ...
      >
        ...
        <div className="flex flex-col items-center gap-2">
          ...
          <p className="text-sm text-muted-foreground">
            {uploading ? "Subiendo archivo..." : "Arrastre un archivo aqui o"}
          </p>
          {!uploading && (
      ...
      {files.length > 0 && (
        <div className="flex flex-col sm:flex-row gap-2">
          ...
          <Select value={categoryFilter} onValueChange={setCategoryFilter}>
            ...
            <SelectContent>
              {Object.entries(UI.rrhh.staffDocCategories).map(
                ([value, label]) => (
                  <SelectItem key={value} value={value}>
                    {label}
                  </SelectItem>
          ...
          <div className="flex items-center gap-2 ml-auto">
            ...
            <DropdownMenu>
              ...
              <DropdownMenuContent align="end" className="w-56">
                {UI.rrhh.staffDocDefaultLabels.map((label) => (
                  <DropdownMenuItem
                    key={label}
      ...
      {files.length === 0 ? (
        <EmptyState
          ...
          description="Este miembro del equipo no tiene documentos adjuntos."
        />
      ) : filteredFiles.length === 0 ? (
        <EmptyState
          icon={Search}
        ...
        <Card className="py-0 gap-0 overflow-hidden">
          <div className="divide-y">
            ...
            {filteredFiles.map((file) => {
              ...
              const categoryLabel =
                UI.rrhh.staffDocCategories[file.category] || file.category;
            ...
            <div className="flex items-center gap-3 min-w-0">
              ...
              <div className="min-w-0">
                ...
                <p className="text-[11px] text-muted-foreground">
                  {previewFile && formatDate(previewFile.createdAt)}
      ...
      >
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Agregue o modifique la etiqueta del archivo. Deje vacio para
              quitar.
            </DialogDescription>
          </DialogHeader>
          ...
          <Input
            ...
            placeholder="Ej: CV Actualizado, Contrato Vigente..."
            maxLength={100}
          />
          <DialogFooter>
            <Button
      ...
      >
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Seleccione la nueva categoria para &quot;
